...
The mean is **`r ac_mean`** and the median is **`r ac_median`**
...
```
The interval **`r maxint[1]`** has the maximum number of steps on average (**`r maxint[2]`**)
...
The total number of missing values in the dataset is **`r missvalues`**
...
Now activityfull has **`r sum(is.na(activityfull$steps))`** rows with NA
...
The new mean is **`r acfull_mean`** and the median is **`r acfull_median`**. Imputing the missing data impact to the median not to the mean.
